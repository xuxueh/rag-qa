"""终极组合评测：混合检索（召回 top-10）→ rerank（精排 top-5）
这是生产环境的完整链路：向量 + BM25 找得多 → reranker 排得准
用法：python eval_hybrid_rerank.py
"""
import sys
import os
import json
...
# 4. 评测：混合检索召回 top-10 → rerank 精排 top-5
hits = {"top1": 0, "top3": 0, "top5": 0}
for item in eval_set:
    q = item["question"]
    target = item["source"]
    recall = retriever.retrieve(q, top_k=10)       # ① 混合召回 top-10
    ranked = rerank(q, recall, top_n=5)            # ② rerank 精排 top-5
...
total = len(eval_set)
print("=" * 50)
print(f"评测集：{total} 条 | 流程：混合检索(向量+BM25) → rerank → top-5")
...
print(f"  混合+Rerank（本结果）  Top-1 {hits['top1']/total:.1%} | Top-3 {hits['top3']/total:.1%} | Top-5 {hits['top5']/total:.1%}")
